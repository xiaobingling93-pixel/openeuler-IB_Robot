...
#    (此时 --arm 参数被忽略)
ros2 run so101_hw_interface so101_read_steps --raw --rate 1

# 2. 读取 "原始" (0-4095) 步数值 (一次)
ros2 run so101_hw_interface so101_read_steps --raw --once

# 3. 读取 Follower 臂的 "校准后" (归一化) 值 (连续, 默认)
ros2 run so101_hw_interface so101_read_steps --arm follower --rate 1

# 4. 读取 Leader 臂的 "校准后" (归一化) 值 (一次)
ros2 run so101_hw_interface so101_read_steps --arm leader --once
"""
